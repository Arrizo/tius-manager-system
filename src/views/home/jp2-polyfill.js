...
loadOpenJPEG = function () {
  ...
  let script = new URL("./openjpeg.js", import.meta.url).href;
  console.log(import.meta, script, 1111);
  s.src = script;
...
function createCanvasFromUrl(src, displayStats) {
  ...
  xhr.onreadystatechange = function () {
    ...
    if (xhr.status !== 200 && xhr.status !== 0) {
      throw (
        "Unable to load image data: XHR status " + xhr.status + " for " + src
      );
    }
    ...
    if (!!displayStats) {
      ...
      var message =
        "Decode: " +
        canvas.polyfillDecodeSeconds.toFixed(2) +
        "s; " +
        "Render: " +
        canvas.polyfillRenderSeconds.toFixed(2) +
        "s";
